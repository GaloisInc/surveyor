...
-- filename
asynchronouslyLoad :: NG.NonceGenerator IO s -> C.Chan (SCE.Events s st) -> FilePath -> IO AsyncLoader
...
-- | Load a JAR file
asynchronouslyLoadJAR :: NG.NonceGenerator IO s -> C.Chan (SCE.Events s st) -> FilePath -> IO AsyncLoader
asynchronouslyLoadJAR ng customEventChan jarPath = do
  ...
  errThread <- A.async $ do
    worker <- A.async $ do
      ...
      SCE.emitEvent customEventChan (SCE.AnalysisFinished (A.SomeResult lastRes) [])
    C.putMVar mv worker
    eres <- A.waitCatch worker
    case eres of
      Right () -> return ()
      Left exn -> SCE.emitEvent customEventChan (SCE.AnalysisFailure exn)
  ...
  where
    addJARChunk jr res classNames = do
      ...
      SCE.emitEvent customEventChan (SCE.AnalysisProgress (A.SomeResult res'))
      return res'

asynchronouslyLoadLLVM :: NG.NonceGenerator IO s -> C.Chan (SCE.Events s st) -> FilePath -> IO AsyncLoader
asynchronouslyLoadLLVM ng customEventChan bcPath = do
  ...
  errThread <- A.async $ do
    worker <- A.async $ do
      ...
      case em of
        Left err ->
          SCE.emitEvent customEventChan (SCE.ErrorLoadingLLVM (LL.formatError err))
        Right m -> do
          nonce <- NG.freshNonce ng
          hdlAlloc <- CFH.newHandleAllocator
          llvmRes <- A.mkLLVMResult ng nonce hdlAlloc m
          SCE.emitEvent customEventChan (SCE.AnalysisFinished llvmRes [])
    C.putMVar mv worker
    eres <- A.waitCatch worker
    case eres of
      Right () -> return ()
...
asynchronouslyLoadElf :: NG.NonceGenerator IO s -> C.Chan (SCE.Events s st) -> FilePath -> IO AsyncLoader
